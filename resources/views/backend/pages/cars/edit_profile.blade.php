...
@section('main-section')
                        ...
                        <form id="profileformpart2" method="post" action="{{ url('update_profile_part_2') }}">
                            ...
                            <div class="row mb-2">
                                <div class="col-sm-6 text-white"><strong class="text-warning">Mileage:</strong>
                                    <input type="text" value="{{ $car_data->mileage }}" class="form-control  bg-dark text-white "
                                        name="mileage">
                                </div>
    ...
    <select name="fuel_type" class="form-select bg-dark text-white border-0 mt-1">
        ...
        {{-- Single Fuel Options --}}
        <option value="Petrol" {{ $car_data->fuel_type == 'Petrol' ? 'selected' : '' }}>Petrol</option>
        <option value="Diesel" {{ $car_data->fuel_type == 'Diesel' ? 'selected' : '' }}>Diesel</option>
        <option value="Electric" {{ $car_data->fuel_type == 'Electric' ? 'selected' : '' }}>Electric (EV)</option>
        
        {{-- Combination Options --}}
        <option value="Petrol + CNG" {{ $car_data->fuel_type == 'Petrol + CNG' ? 'selected' : '' }}>Petrol + CNG</option>
        <option value="Petrol + Hybrid" {{ $car_data->fuel_type == 'Petrol + Hybrid' ? 'selected' : '' }}>Petrol + Hybrid</option>
        <option value="Diesel + Hybrid" {{ $car_data->fuel_type == 'Diesel + Hybrid' ? 'selected' : '' }}>Diesel + Hybrid</option>
        <option value="CNG Only" {{ $car_data->fuel_type == 'CNG Only' ? 'selected' : '' }}>CNG Only</option>
    </select>
</div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-sm-6 text-white"><strong class="text-warning">Seats:</strong>
                                    <input type="text" value="{{ $car_data->seat_capacity }}" class="form-control  bg-dark text-white"
                                        name="seats">
                                </div>
                                <div class="col-sm-6 text-white"><strong class="text-warning">Engine:</strong>
                                    <input type="text" value="{{ $car_data->engine }}" class="form-control  bg-dark text-white"
                                        name="engine">
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-sm-6 text-white"><strong class="text-warning">Registration
                                        Number:</strong>
                                    <input type="text" value="{{ $car_data->registration_no }}" class="form-control  bg-dark text-white" 
                                        name="registration_no">
                                </div>

                                <div class="col-sm-6 text-white"><strong class="text-warning">Owner Name:</strong>
                                    <input type="text" value="{{ $car_data->owner_name }}" class="form-control  bg-dark text-white"
                                        name="owner_name">
                                </div>
                            ...
                            <div class="row mb-2">
                                <div class="col-sm-6 text-white"><strong class="text-warning">Location:</strong>
                                    <input type="text" value="{{ $car_data->location }}" class="form-control  bg-dark text-white"
                                        name="location">
                                </div>
                                ...
                                <div class="col-sm-6 bg-dark text-white"><strong class="text-warning">Duplicate Keys:</strong>
...
<div class="container">
    ...
    <form id="update_main_doc_form" method="post" enctype="multipart/form-data" action="{{ url('save_imp_docs') }}">
        @csrf
        <input type="hidden" value="{{ $car_data->unique_id }}" name="car_id">
        ...
        <div class="row">
            ...
            <div class="col-4 mb-3">
                ...
                @if(!empty($car_data->rc_book))
                    <a href="{{ asset($car_data->rc_book) }}" target="_blank" class="btn btn-sm btn-primary mt-2">View File</a>
                @else
            ...
            <div class="col-4 mb-3">
                ...
                @if(!empty($car_data->pollution))
                    <a href="{{ asset($car_data->pollution) }}" target="_blank" class="btn btn-sm btn-primary mt-2">View File</a>
                @else
                    <p class="text-muted mt-2">No file uploaded</p>
                @endif
            </div>
            ...
            <div class="col-4 mb-3">
                ...
                @if(!empty($car_data->insurance))
                    <a href="{{ asset($car_data->insurance) }}" target="_blank" class="btn btn-sm btn-primary mt-2">View File</a>
                @else
                    <p class="text-muted mt-2">No file uploaded</p>
                @endif
            </div>
            ...
            <div class="col-12 mt-3">
                <button class="btn btn-success" onclick="update_main_doc()" type="button">Update It</button>
            </div>
        </div>
    </form>
        ...
        <div class="table-responsive">
            <table class="table table-dark table-striped table-bordered align-middle">
                <thead>
                    ...
                    <form id="newservicing_form" method="post" action="{{ url('add_servicing') }}" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" value="{{ $id }}" name="car_id">
                        <tr>
                            <td>
                                <label class="small">Service type</label>
                                <select name="service_master" class="form-control form-control-sm" required>
                                    <option selected disabled>Please Select</option>
                            ...
                            <td>
                                ...
                                <input type="number" class="form-control form-control-sm" name="total_amount" id="add_total">
                            </td>
                            <td>
                                <label class="small">Amount Paid</label>
                                <input type="number" class="form-control form-control-sm" name="amount_paid" id="add_paid">
                            </td>
                            <td>
                                <label class="small">Due Amount</label>
                                <input type="number" class="form-control form-control-sm bg-secondary text-white" name="due_amount" id="add_due" readonly>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <input type="file" class="form-control form-control-sm" name="invoice[]" multiple>
                            </td>
                            <td>
                                <button id="add_btn" onclick="uploadnew_servicing()" type="button" class="btn btn-success btn-sm mt-3 w-100 fw-bold">Add</button>
                            </td>
                        </tr>
                    </form>
                </thead>
                ...
                <tbody>
                    @foreach ($car_service_data as $service)
                        <tr>
                            ...
                            <td>
                                ...
                                @if(!empty($images))
                                    @foreach ($images as $img)
                                        <a target="_blank" href="{{ asset($img) }}" class="d-inline-block mb-1">
                                            <span class="badge bg-info text-dark">Bill {{ $sl++ }}</span>
                                        </a>
                                    @endforeach
                                @else
                            ...
                            <td>
                                <div class="btn-group">
                                    <button class="btn btn-xs btn-warning text-dark" data-bs-toggle="modal" data-bs-target="#editModal{{ $service->id }}">Edit</button>
                                    <button id="del_btn{{ $service->id }}" onclick="delete_servicing({{ $service->id }})" class="btn btn-xs btn-danger">Del</button>
                                    
                                    <form id="deleteForm{{ $service->id }}" action="{{ url('delete_btn_servicing') }}" method="POST" style="display:none;">
                                        @csrf
                                ...
                                <div class="modal fade" id="editModal{{ $service->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content bg-dark text-white border border-warning">
                                            <div class="modal-header border-warning">
                                                <h5 class="modal-title text-warning">Update Service Record</h5>
                                                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                            </div>
                                            <form id="updateForm{{ $service->id }}" action="{{ url('update_servicing') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="table_id" value="{{ $service->id }}">
                                                ...
                                                <div class="modal-body">
                                                    <div class="row g-3">
                                                        <div class="col-md-4">
                                                            ...
                                                            <select name="service_master" class="form-control">
                                                                @foreach ($service_master as $st)
                                                                    <option value="{{ $st->id }}" {{ $service->service_type_id == $st->id ? 'selected' : '' }}>{{ $st->service_type }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Date</label>
                                                            <input type="date" class="form-control" name="service_date" value="{{ $service->billed_on_date }}">
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Garage</label>
                                                            <select name="garage" class="form-control">
                                                                @foreach ($garage_master as $g)
                                                                    <option value="{{ $g->id }}" {{ $service->garage_id == $g->id ? 'selected' : '' }}>{{ $g->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Total Cost</label>
                                                            <input type="number" class="form-control edit-total" name="total_amount" value="{{ $service->cost }}">
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Paid</label>
                                                            <input type="number" class="form-control edit-paid" name="amount_paid" value="{{ $service->bill_paid }}">
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Due</label>
                                                            <input type="number" class="form-control edit-due bg-secondary text-white" name="due_amount" value="{{ $service->due }}" readonly>
                                                        </div>
                                                        <div class="col-md-4">
                                                            <label>Status</label>
                                                        ...
                                                        <div class="col-md-8">
                                                            <label>Invoice <small class="text-warning">(Uploading new files will delete old ones)</small></label>
                                                            <input type="file" class="form-control" name="invoice[]" multiple>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal-footer border-warning">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                    <button id="upd_btn{{ $service->id }}" onclick="update_servicing({{ $service->id }})" type="button" class="btn btn-warning text-dark fw-bold">Save Changes</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
...
// For Add Form
handleCalculation(document.getElementById('add_total'), document.getElementById('add_paid'), document.getElementById('add_due'));

// For All Edit Modals
document.querySelectorAll('.modal').forEach(modal => {
    handleCalculation(modal.querySelector('.edit-total'), modal.querySelector('.edit-paid'), modal.querySelector('.edit-due'));
});
...
function uploadnew_servicing() {
    ...
    .then(data => {
        if (data.success) {
            Swal.fire({ icon: 'success', title: 'Added!', text: data.message, timer: 1500 }).then(() => location.reload());
        } else {
...
function update_servicing(id) {
    ...
    .then(data => {
        if (data.success) {
            Swal.fire({ icon: 'success', title: 'Updated!', text: data.message, timer: 1500 }).then(() => location.reload());
        } else {
            Swal.fire('Error', data.message, 'error');
            setBtnState(btn, 'reset');
        }
				...
				{{--  End of manage service recoreds --}}
                        ...
                        <ul class="mb-0">
                            ...
                            <form method="POST" id="additional_detail_form" action="{{ url('additional_info_car') }}">
                                ...
                                <!-- Submit Button -->
                                <button type="button" onclick="update_additional_details()"
                                    class="btn btn-success mt-3">Update
                                    it</button>
                            </form>
